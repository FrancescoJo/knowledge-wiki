...
import com.fj.omnimemo.infrastructure.note.persistence.NoteVersionRepositoryImpl
...
class NoteVersionRepositoryImplTest {
    ...
    inner class SaveContent {
        ...
        @Test
        fun `version 2 without prior snapshot is stored as delta from empty base`() {
            repo.saveContent(testNote.id, 2, "# Hello")
    ...
    inner class FindContent {
        ...
        @Test
        fun `should return null when target version has a gap in delta history`() {
            repo.saveContent(testNote.id, 1, "v1")
            repo.saveContent(testNote.id, 2, "v2")

            repo.findContent(testNote.id, 3) shouldBe null
        }
    }
