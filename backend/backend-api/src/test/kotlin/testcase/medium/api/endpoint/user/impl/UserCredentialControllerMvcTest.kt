...
class UserCredentialControllerMvcTest {
    ...
    @Test
    fun `PUT users email returns 400 for malformed user ID`() {
        mockMvc.perform(
            put("${ApiPathsV1.USERS}/not-a-uuid/email")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"email":"new@example.com"}""")
        )
    ...
    @Test
    fun `PUT users password returns 400 for malformed user ID`() {
        mockMvc.perform(
            put("${ApiPathsV1.USERS}/not-a-uuid/password")
                .contentType(MediaType.APPLICATION_JSON)
                .content("""{"password":"newpass"}""")
        )
            .andExpect(status().isBadRequest)
